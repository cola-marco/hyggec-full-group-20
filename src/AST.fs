...
type Position =
    { /// "Main" position of the AST elements, used e.g. to report typing errors.
      Main: Lexer.Position
      /// Position where the AST element begins.
      Begin: Lexer.Position
      /// Position where the AST element ends.
      End: Lexer.Position
    }
    with
        ...
        /// source file.
        member this.Format =
           $"(%d{this.Begin.Line}:%d{this.Begin.Column}-%d{this.End.Line}:%d{this.End.Column})"
...
and Expr<'E,'T> =
    ...
    | BinNumOp of op: NumericalOp
                * lhs: Node<'E,'T>
                * rhs: Node<'E,'T>

    /// Binary logical operation between lhs and rhs.
    | BinLogicOp of op: LogicOp
                  * lhs: Node<'E,'T>
                  * rhs: Node<'E,'T>
