...
let rec internal typer (env: TypingEnv) (node: UntypedAST): TypingResult =
    ...
    | BinNumOp(op, lhs, rhs) ->
        match op with
        | NumericalOp.Add ->
            match (binaryNumOpTyper "addition" node.Pos env lhs rhs) with
            | Ok(tpe, tlhs, trhs) ->
    ...
    | BinLogicOp(op, lhs, rhs) ->
        match op with
        | LogicOp.And ->
            match (binaryBoolOpTyper "and" node.Pos env lhs rhs) with
            | Ok(tlhs, trhs) ->
                Ok { Pos = node.Pos; Env = env; Type = TBool; Expr = BinLogicOp(op, tlhs, trhs) }
            | Error(es) -> Error(es)
        | LogicOp.Or ->
            match (binaryBoolOpTyper "or" node.Pos env lhs rhs) with
            | Ok(tlhs, trhs) ->
                Ok { Pos = node.Pos; Env = env; Type = TBool; Expr = BinLogicOp(op, tlhs, trhs) }
            | Error(es) -> Error(es)
        | LogicOp.Xor ->
    ...
    | BinRelOp(op, lhs, rhs) ->
        match op with
        | RelationalOp.Eq ->
            match (binaryNumRelOpTyper "equal to" node.Pos env lhs rhs) with
            | Ok(tlhs, trhs) ->
        ...
        | RelationalOp.Less ->
            ...
            | Error(es) -> Error(es)
        | RelationalOp.LessEq ->
            match (binaryNumRelOpTyper "less than or equal to" node.Pos env lhs rhs) with
            | Ok(tlhs, trhs) ->
                Ok { Pos = node.Pos; Env = env; Type =TBool; Expr = BinRelOp(op, tlhs, trhs) }
            | Error(es) -> Error(es)
        ...
        if not dups.IsEmpty then
            ...
            | Ok(texpr) ->
                ...
                | TUnion(unionCases) ->
                    ...
                    /// The function 'caseTyper' is mapped over all 'cases': it looks
                    /// for the matched label in 'unionLabels', extracts the
                    /// corresponding type from 'unionTypes', and type-checks the
                    /// continuation by introducing the matched variable and type in
                    /// the environment.
                    let caseTyper (label, v, cont: UntypedAST): TypingResult =
                        match (List.tryFindIndex (fun l -> l = label) unionLabels) with
                        | Some(i) ->
                            /// Updated environment for type-checking the union case continuation
                    ...
                    if not errors.IsEmpty then Error(errors)
                        ...
                        /// Types of all match continuations
                        let contTypes = List.map (fun (c: TypedAST) -> c.Type) typedConts

                        match (lubMany env contTypes) with
                        | Some(matchType) ->
                        ...
                        | None ->
                            let errMsg =
                                $"pattern match result type mismatch: could not compute a least upper bound for %s{Util.formatSeq contTypes}"
                            Error([(node.Pos, errMsg)])
                | _ ->
...
/// return type errors.
and internal binaryNumOpTyper descr pos (env: TypingEnv)
                              (lhs: UntypedAST)
...
/// errors.
and internal binaryBoolOpTyper descr pos (env: TypingEnv)
                               (lhs: UntypedAST)
...
/// return type errors.
and internal binaryNumRelOpTyper descr pos (env: TypingEnv)
                                 (lhs: UntypedAST)
                                 (rhs: UntypedAST): Result<TypedAST * TypedAST, TypeErrors> =
    let tlhs = typer env lhs
    let trhs = typer env rhs
