...
let tests = testList "tests" [
    createTestList "lexer"
        <| fun file ->
            Expect.isOk (Lexer.tokenize file) "Lexing failed"
        <| fun file ->
            Expect.isError (Lexer.tokenize file) "Lexing should have failed"
    createTestList "parser"
        <| fun file ->
            // Expect.isOk (Util.parseFile file) "Parsing failed"
            match (Lexer.tokenize file) with
            | Error(e) -> failwith $"Lexing failed: %s{e}"
            | Ok(tokens) -> Expect.isOk (Parser.parse tokens) "Parsing failed"
        <| fun file ->
            // Expect.isError (Util.parseFile file) "Parsing should have failed"
            match (Lexer.tokenize file) with
            | Error(e) -> failwith $"Lexing failed: %s{e}"
            | Ok(tokens) -> Expect.isError (Parser.parse tokens) "Parsing should have failed"
