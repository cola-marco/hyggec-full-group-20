...
let rec substVar (node: Node<'E,'T>) (var: string) (var2: string): Node<'E,'T> =
    ...
    | BinNumOp(op, lhs, rhs) ->
        {node with Expr = BinNumOp(op, (substVar lhs var var2), (substVar rhs var var2))}

    | BinLogicOp(op, lhs, rhs) ->
        {node with Expr = BinLogicOp(op, (substVar lhs var var2), (substVar rhs var var2))}
    | Not(arg) ->
        {node with Expr = Not(substVar arg var var2)}

    | BinRelOp(op, lhs, rhs) ->
        {node with Expr = BinRelOp(op, (substVar lhs var var2), (substVar rhs var var2))}
...
let rec internal toANFDefs (node: Node<'E,'T>): Node<'E,'T> * ANFDefs<'E,'T> =
    ...
    | BinNumOp(op, lhs, rhs) ->
        ...
        let anfExpr = BinNumOp(op, lhsANF, rhsANF)
    ...
    | BinLogicOp(op, lhs, rhs) ->
        ...
        let (rhsANF, rhsDefs) = toANFDefs rhs
        /// This expression in ANF
        let anfExpr = BinLogicOp(op, lhsANF, rhsANF)
        /// Definition binding this expression in ANF to its variable
        let anfDef = ANFDef(false, {node with Expr = anfExpr})
    ...
    | BinRelOp(op, lhs, rhs) ->
        ...
        let (rhsANF, rhsDefs) = toANFDefs rhs
        /// This expression in ANF
        let anfExpr = BinRelOp(op, lhsANF, rhsANF)
        /// Definition binding this expression in ANF to its variable
        let anfDef = ANFDef(false, {node with Expr = anfExpr})
    ...
    | Seq(nodes) ->
        ...
        | last :: rest ->
            ...
            /// We collect the definitions and ignore the variables bound to
            /// each node of the sequence, because such variables will not be
            /// used to construct the sequence returned below.
